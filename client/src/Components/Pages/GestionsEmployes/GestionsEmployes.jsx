...
export const GestionsEmployes = () => {
  ...
  const fetchEmployes = async () => {
    try {
      ...
      const response = await axiosInstance.get(
        `/empresa/empleados/${userSession.ID}`
      );
      ...
      alert(
        "Hubo un problema al cargar los empleados. Por favor, inténtalo más tarde."
      );
    }
  };
  ...
  const handleFilterChange = (e) => {
    ...
    const filteredByState = filtered.filter((employe) => {
      ...
      if (
        ...
        employe.estado.toLowerCase() === "inactivo"
      ) {
        return true;
      }
      return false;
  ...
  const prev = () =>
    setCurrent(
      (prev) => (prev - 1 + filteredEmployes.length) % filteredEmployes.length
    );

  const showModalCreateEmploye = () => {
    const modalCreateEmploye = document.getElementById(
      "modal-overlayCreateEmploye"
    );
  ...
  if (acces_granted) {
    return (
      <>
          ...
          <div className="container_GestionsEmploye">
            ...
            <div className="containerGestionsEmployeOptions">
              <div className="containerConsultEmploye">
                ...
                <button
                  className="btn_createEmploye"
                  onClick={showModalCreateEmploye}
                >
                  Agregar Empleado
                </button>
              </div>
                  ...
                  <div className="carousel-track">
                    ...
                    filteredEmployes.length === 1 ? (
                      <div className="carousel-card card-center">
                        <img
                          src={
                            filteredEmployes[0]?.foto_perfil ||
                            "default-profile.png"
                          } // Imagen del Employe
                          alt="Employe"
                        ...
                        <div className="carousel-card-info">
                          <h3>
                            {filteredEmployes[0]?.nombres}{" "}
                            {filteredEmployes[0]?.apellidos}
                          </h3>
                    ...
                    filteredEmployes.length === 2 ? (
                      [0].map((offset) => {
                        const index =
                          (current + offset) % filteredEmployes.length;
                        ...
                        return (
                          ...
                          >
                            <img
                              src={
                                Employe?.foto_perfil || "default-profile.png"
                              } // Imagen del Employe
                              alt="Employe"
                              className="carousel-image"
                            />
                  ...
                  {filteredEmployes.length > 0 && (
                    <div className="Employe-info">
                      <h3>
                        {
                          filteredEmployes[
                            (current + 1) % filteredEmployes.length
                          ]?.nombres
                        }{" "}
                        {
                          filteredEmployes[
        ...
        {selectedEmploye && <UpdateEmploye Employe={selectedEmploye} />}
